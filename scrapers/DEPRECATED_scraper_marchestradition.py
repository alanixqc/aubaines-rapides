#!/usr/bin/env python3
"""Scraper Les Marchés Tradition — via l'API Flipp (flyers-ng.flippback.com)"""
...
LOCALE = "fr"
POSTAL = "J7Y4A2"  # Saint-Jérôme
MERCHANT_NAME = "Les Marchés Tradition"
...
MEAT_KW = [
    ...
    "longe", "poisson", "saumon", "truite", "crevette",
]
...
def main():
    parser = argparse.ArgumentParser(description="Marchés Tradition Meat Scraper")
    ...
    print(f"📡 {len(flyers)} circulaires trouvées pour {MERCHANT_NAME}", file=sys.stderr)
    ...
    if args.save or args.output:
        out_path = args.output or os.path.join(
            os.path.dirname(__file__), "..", "data", "marchestradition_products.json"
        )
    ...
    if args.db:
        ...
        row = db.execute("SELECT id FROM stores WHERE slug = ? OR name = ?",
                          ("les-marches-tradition", MERCHANT_NAME)).fetchone()
        if row:
            store_id = row["id"]
        else:
            db.execute("INSERT INTO stores (name, slug) VALUES (?, ?)", (MERCHANT_NAME, "les-marches-tradition"))
            db.commit()
        ...
        db.commit()
        db.close()
        print(f"💾 DB: {inserted} produits insérés", file=sys.stderr)
