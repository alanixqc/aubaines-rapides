#!/usr/bin/env python3
"""
Metro Meat & Poultry Scraper
=============================
Scrapes product data from metro.ca meat/poultry aisles using Playwright.

Shares the same Drupal platform as Super C (superc.ca) with identical HTML
data-product-* attributes and CSS class-based pricing sections.
...
Usage:
    python scraper_metro.py                          # scrape all + print JSON
    python scraper_metro.py --save                   # scrape and save to file
    python scraper_metro.py --output out.json        # save to custom path
    python scraper_metro.py --category beef          # scrape only one category
    python scraper_metro.py --headless false         # show browser window
    python scraper_metro.py --store                  # scrape + save to database
...
# Pagination URL pattern (page 2+): /en/online-grocery/aisles/meat-poultry/{slug}-page-{n}
# Page 1 has the base URL, page 2+ appends "-page-N"

DEFAULT_OUTPUT = os.path.join(
    os.path.dirname(__file__), "..", "data", "metro_products.json"
)
...
class MetroScraper:
    """Scrapes meat/poultry products from metro.ca using Playwright."""
    ...
    def _handle_modals(self):
        """
        Handle cookie consent and any other modal overlays on metro.ca.
        Metro uses OneTrust for cookie consent and may show a postal code
        / store selector modal.
        """
        # OneTrust cookie consent (shared with Super C)
        cookie_selectors = [
            ...
            ".onetrust-close-btn-handler button",
        ...
        # Metro-specific: close the postal code / store selector modal
        # This modal has a "close" button or a "Skip" / "I haven't made my choice yet" link
        modal_close_selectors = [
            "button:has-text('I haven't made my choice yet')",
    ...
    def _get_page_products(self) -> list[dict]:
        ...
        products = self._page.evaluate("""() => {
            ...
            function extractProduct(tile) {
                ...
                // Has sale sticker
                const hasSale = !!tile.querySelector('.icon--sale, .pricing__sale-price');
    ...
    def _parse_weight(self, package_info: Optional[str]) -> Optional[str]:
        ...
        cleaned = re.sub(
            r'^(A tray contains on average|Contains approximately|Approx\.|Contains)',
            '',
            package_info,
            flags=re.IGNORECASE,
        ).strip()
    ...
    def _format_product(self, raw: dict, meat_type: str) -> dict:
        ...
        return {
            ...
            "valid_to": None,  # Metro doesn't surface valid_to in HTML
    ...
    def scrape(self, categories: Optional[list[str]] = None) -> dict:
        """
        Scrape product data from Metro.
        ...
        print(f"\n{'='*60}", file=sys.stderr)
        print(f"🛒 METRO — Meat & Poultry Scraper", file=sys.stderr)
    ...
    def store_products(self, products: list[dict]):
        ...
        # Ensure Metro exists in stores
        db.execute(
            "INSERT OR IGNORE INTO stores (name, slug) VALUES (?, ?)",
            (merchant, "metro"),
        )
        store = db.execute(
            "SELECT id FROM stores WHERE slug = ?", ("metro",)
        ).fetchone()

        if not store:
            print("❌ Store 'Metro' not found in DB", file=sys.stderr)
            db.close()
            return
...
def main():
    parser = argparse.ArgumentParser(
        description="Metro Meat & Poultry Scraper",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  python scraper_metro.py                    # scrape all, print JSON to stdout
  python scraper_metro.py --save             # scrape all, save to data/metro_products.json
  python scraper_metro.py --output out.json  # scrape all, save to custom path
  python scraper_metro.py --category beef    # scrape only beef
  python scraper_metro.py --category beef,chicken  # scrape beef and chicken
  python scraper_metro.py --headless false   # show browser window
  python scraper_metro.py --store            # scrape + save to database
        """,
    )
    ...
    # Run scraper
    try:
        with MetroScraper(headless=headless, delay=args.delay) as scraper:
